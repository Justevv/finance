...
import com.manager.finance.domain.repository.ExpenseRepository;
...
import com.manager.finance.infrastructure.cache.redis.ExpenseRedisRepository;
...
public class ExpenseService implements CrudService<ExpenseRequestDTO, ExpenseResponseDTO> {
    ...
    @Transactional
    @Override
    public ExpenseResponseDTO create(Principal principal, ExpenseRequestDTO expenseRequestDTO) {
        log.debug(crudLogConstants.getInputNewDTO(), expenseRequestDTO);
        var expense = mapper.map(expenseRequestDTO, ExpenseEntity.class);
        expense.setUser(userHelper.getUser(principal));
        expense.setDate(LocalDateTime.now());
        expense.setCategory(categoryService.getOrCreate(principal, expenseRequestDTO.getCategory()));
        expense.setPlace(placeService.getOrCreate(expenseRequestDTO.getPlace()));
    ...
    @Transactional
    @Override
    public ExpenseResponseDTO update(UUID id, Principal principal, ExpenseRequestDTO expenseRequestDTO) {
        log.debug(crudLogConstants.getInputDTOToChangeEntity(), expenseRequestDTO, null);
        ...
        mapper.map(expenseRequestDTO, updatedExpense);
